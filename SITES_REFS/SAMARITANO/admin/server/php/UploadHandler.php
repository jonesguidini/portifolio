...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
			'database' => 'samarita_site',
			'host' => 'localhost',
			'username' => 'root',
			'password' => '',
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/../../fotos_uploads/'.$options['id_galeria'].'/',
            'upload_url' => $this->get_full_url().'/../../fotos_uploads/'.$options['id_galeria'].'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    //$sql = mysql_query("INSERT INTO fotosevento (IdEvento, foto) VALUES ('{$this->options['id_galeria']}', '{$file->name}')") or die(mysql_error());
            ...
            foreach($this->options['image_versions'] as $version => $options) {
                if (!empty($version)) {
                    ...
                    if (is_file($file)) {
                    	...
                    	$sql = mysql_query("DELETE FROM fotosevento WHERE foto = '{$file_name}'") or die(mysql_error());
